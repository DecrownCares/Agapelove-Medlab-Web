        ...
        data.forEach(categoryData => {
          ...
          if (headlines.length > 0) {
            ...
            headlines.forEach(headline => {
              ...
              titleContainer.className = "ticker-title";
              const link = document.createElement("a");
              link.href = `/post.html?slug=${headline.slug}`;
              link.textContent = headline.title;
              titleContainer.appendChild(link);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Example post data (replace this with real data fetching logic)
const post = {
    meta: {
        description: "This is a dynamically generated meta description for a post.",
        keywords: "dynamic, post, blog, keywords",
        ogTitle: "Dynamic OG Title for Post",
        ogDescription: "OG description for the dynamic post.",
        ogImage: "https://example.com/default-image.jpg",
        twitterTitle: "Twitter Title for Post",
        twitterDescription: "Twitter description for the dynamic post.",
...
function updateMetaTags(meta) {
    ...
    const metaInfo = [
        ...
        { 'http-equiv': 'last-modified', content: meta.lastModified }
    ];

    metaInfo.forEach(info => {
        let metaTag = document.querySelector(`meta[${info.name ? `name="${info.name}"` : `property="${info.property}"`}]`);
        if (!metaTag) {
            metaTag = document.createElement('meta');
            if (info.name) {
                metaTag.setAttribute('name', info.name);
            } else if (info.property) {
                metaTag.setAttribute('property', info.property);
            }
            head.appendChild(metaTag);
        }
        metaTag.setAttribute('content', info.content);
    });
    ...
    if (meta.canonicalUrl) {
        ...
        linkTag.setAttribute('href', meta.canonicalUrl);
    }
}

// Call the function with post meta data
...
document.getElementById("contactForm").addEventListener("submit", function (event) {
    event.preventDefault(); // Prevent form submission to handle validation and AJAX request
    ...
    if (!name || !email || !subject || !message) {
        messageElement.textContent = "All fields marked with * are required!";
        messageElement.classList.remove("success");
        messageElement.classList.add("error");
        setTimeout(() => (messageElement.textContent = ""), 3000);
        return;
    ...
    fetch('https://infohub-ffez.onrender.com/api/contact/submit-contact-form', {
        method: 'POST', 
        headers: {
            'Content-Type': 'application/json'
        },
        ...
        .catch((error) => {
            messageElement.textContent = "There was an error sending your message. Please try again later.";
            messageElement.classList.remove("success");
            messageElement.classList.add("error");
            setTimeout(() => (messageElement.textContent = ""), 3000);
